var _carbonads = {
    init: function (where, force_serve) {
        ...
        var placement = this.getUrlVar('placement'),
            protocol = this.getUrlVar('protocol') ? this.getUrlVar('protocol') + ':' : '',
        ...
        r['slipsumcom'] = 'CKYI527U';
        r['ui-cloudcom'] = 'CKYIPK3E';
        if (this.isset(r[placement]) && (serve == 'CVYD42T' || serve == 'CVYD42E' || serve == 'C6AILKT')) return r[placement]; else if (serve == 'CVYD42T' || serve == 'CVYD42E' || serve == 'C6AILKT') return 'CKYICKQI'; else return serve
...
function _carbonads_go(b) {
    ...
    var protocol = _carbonads.getUrlVar('protocol') ? _carbonads.getUrlVar('protocol') + ':' : '';
    ...
    if (ad.fetch != null) {
        ...
        fetch.src = ad.fetch.startsWith('//') ? (protocol + ad.fetch) : ad.fetch;
    ...
    if (!_carbonads.nonempty(ad.statlink) && (!(_carbonads.nonempty(ad.fallbackImage) && _carbonads.nonempty(ad.fallbackLink) && _carbonads.nonempty(ad.fallbackTitle)) || !(_carbonads.nonempty(ad.fallbackZoneKey)))) {
        ...
        fallback.src = _carbonads._buildSrvUrl(protocol + '//srv.carbonads.net/ads/' + fallbackKey + '.json?segment=placement:' + placement + '&callback=_carbonads_go');
    ...
    if (typeof link[1] != 'undefined' && _carbonads.isset(ad.statlink)) fulllink = link[0] + '?segment=placement:' + placement + ';&encredirect=' + encodeURIComponent(link[1]); else if (link[0].search('srv.buysellads.com') > 0 && _carbonads.isset(ad.statlink)) fulllink = link[0] + '?segment=placement:' + placement + ';'; else if (Array.isArray(link)) fulllink = link[0]; else fulllink = link;
    fulllink = fulllink.replace(/srv.buysellads.com/g, 'srv.carbonads.net');
    var el = document.createElement('span');
    el.innerHTML = '<span class="carbon-wrap"><a href="https:' + fulllink.replace('[timestamp]', time) + '" class="carbon-img" target="_blank" rel="noopener"><img src="' + image + '" alt="" border="0" height="100" width="130" /></a><a href="https:' + fulllink.replace('[timestamp]', time) + '" class="carbon-text" target="_blank" rel="noopener">' + _carbonads.htmlEncode(description) + '</a></span>';
    if (!_carbonads.isset(ad.removecarbon)) el.innerHTML += '<a href="http://carbonads.net/?utm_source=' + placement + '&utm_medium=ad_via_link&utm_campaign=in_unit&utm_term=carbon" class="carbon-poweredby" target="_blank" rel="noopener">ads via Carbon</a>';
    if (typeof ad.pixel != 'undefined') {
        var pixels = ad.pixel.split('||');
        for (var j = 0; j < pixels.length; j++) {
            var pix = document.createElement('img');
            var pixSrc = pixels[j].replace('[timestamp]', time);
            pix.src = pixSrc.startsWith('//') ? (protocol + pixSrc) : pixSrc;
            pix.border = '0';
            pix.height = '1';
            pix.width = '1';
